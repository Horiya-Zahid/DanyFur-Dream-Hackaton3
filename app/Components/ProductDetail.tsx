...
const ProductDetail = ({ product }: { product: Product }) => {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center animate-fadeIn pb-9">
      <div className="flex justify-center items-center">
        <div className="w-full max-w-3xl bg-white p-4 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
           {/* Discount Badge (Dynamically from Sanity) */}
           {/* {product.discountPercentage && (
    <span className="absolute top-5 right-5 bg-red-600 text-white text-lg font-bold px-4 py-2 rounded-lg shadow-lg z-10">
  ...
  {/* ✅ Discount Badge Directly on Image */}
  {product.discountPercentage && (
    <span className="absolute top-3 left-3 bg-red-600 text-white text-lg font-bold px-4 py-2 rounded-lg shadow-lg z-20">
      {product.discountPercentage}% OFF
    </span>
  )}
  ...
  />
      ...
      <div className="flex flex-col justify-between bg-white p-8 rounded-lg shadow-md space-y-6 animate-slideInRight">
        ...
        <p className="text-3xl text-gray-600 font-semibold">$ {product.price}</p>

        <p className="text-gray-600">
          <span className="font-medium">Stock Level: </span>
          {product.stockLevel > 0 ? (
            <span className="text-green-600">{product.stockLevel} Pieces Available</span>
          ) : (
